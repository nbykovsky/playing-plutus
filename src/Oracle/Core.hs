...
data Oracle = Oracle
    { oSymbol   :: !CurrencySymbol  -- currency symbol of NFT (token name will be an empty stirng)
    , oOperator :: !PubKeyHash      -- owner of the oracle
    , oFee      :: !Integer
    , oAsset    :: !AssetClass      -- target currency
...
updateOracle oracle x = do
    ...
    case m of
        Nothing -> do
            ...
            awaitTxConfirmed $ txId $ fromRight (error()) ledgerTx
...
findOracle oracle = do
    ...
    return $ case Map.toList utxos of
        [(oref, o)] -> do
            x <- oracleValue (toTxOut o) $ \dh -> Map.lookup dh $ txData $ toTxOut o
            return (oref, o, x)
        _           -> Nothing
  where
    f :: ChainIndexTxOut -> Bool
    f o = assetClassValueOf (txOutValue $ toTxOut o) (oracleAsset oracle) == 1
